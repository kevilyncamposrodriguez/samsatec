...
class UpdateProductComponent extends Component
{
    ...
    public $refresh, $product_id, $stock_basde = 1, $file_import, $allProducts,  $allCounts = [], $ivai;

    //datos generales
    public $isOpen = 0, $message, $updateMode = false, $ante;
    protected $listeners = ['refreshProducts' => '$refresh', 'changeSkuUP', 'editProduct' => 'edit', 'deleteProduct' => 'delete'];
    ...
    public function render()
    {
        ...
        $teamUser = DB::table("team_user")->select("*")->where("team_id", Auth::user()->currentTeam->id)->where("user_id", Auth::user()->id)->first();
        return view('livewire.product.update-product-component', ['teamUser' => $teamUser]);
    }
    public function importProducts()
    {
        ...
        try {
            if ($this->file_import) {
                $mensaje = Excel::iport(new ProductImport, $this->file_import);
    ...
    public function edit($id)
    {
        ...
        $this->name_family = Family::find($this->id_family)->name;
        $this->name_category = Category::find($this->id_category)->name;
        ...
        try {
            if ($this->product_id) {
                ...
                if ($this->type) {
                    ...
                    $costAnterior = $product->cost_unid;
                    $product->update([
                        ...
                        'stock' => ($product->stock - $product->stock_start) + $this->stock,
                    ...
                    if ($stockAnterior != $this->stock || $costAnterior != $this->cost_unid) {
                        $d_bs = DiaryBook::where('entry', 'like', 'II-%')->where('document', $product->id)->get();
                        foreach ($d_bs as $key => $d_b) {
                            $d_b->update([
                                'debit' => ($d_b->debit != 0) ? $this->stock * $this->cost_unid : 0,
                                'credit' => ($d_b->credit != 0) ? $this->stock * $this->cost_unid : 0
                            ]);
                        }
                    }
                } else {
                ...
                $this->updateMode = false;
                $this->resetInputFields();
                $this->dispatchBrowserEvent('productU_modal_hide', []);
                $this->dispatchBrowserEvent('messageData', ['messageData' => 'Producto o servicio actualizado con exito', 'refresh' => 0]);
            }
