    ...
    <div class="relative">
        <div class="flex items-center justify-between mb-1">
                ...
                @if($this->searchableColumns()->count())
                    <div class="flex rounded-lg w-96 shadow-sm">
                        <div class="relative flex-grow focus-within:z-10">
                            ...
                            <input wire:model.debounce.500ms="search" class="w-full pl-10 py-2 text-sm leading-2 block rounded-md border-gray-300 shadow-sm focus:border-blue-300 focus:ring focus:ring-blue-200 focus:ring-opacity-50 focus:outline-none" placeholder="{{__('Search in')}} " type="text" />
            ...
            @if($this->activeFilters)
                <span class="text-xl text-blue-400 uppercase">@lang('Filtros Activos')</span>
            @endif
            ...
            <div class="flex flex-wrap items-center space-x-1">
                ...
                @if($this->activeFilters)
                    <button wire:click="clearAllFilters" class="flex items-center px-3 py-2 border border-orange-darker rounded-md bg-orange-darker text-white text-xs leading-4 font-medium uppercase tracking-wider hover:bg-red-200 focus:outline-none"><span>{{ __('Reset') }}</span>
                ...
                @if($exportable)
                    <div x-data="{ init() {
                        ...
                        <button wire:click="export" class="flex items-center px-3 py-2 border border-green-400 rounded-md bg-green-600 text-white text-xs leading-4 font-medium uppercase tracking-wider hover:bg-green-800 focus:outline-none"><span>{{ __('Export') }}</span>
            ...
            <div>
                <div class="table min-w-full align-middle">
                    @unless($this->hideHeader)
                        <div class="table-row divide-x divide-blue-200">
                            @foreach($this->columns as $index => $column)
                                @if($hideable === 'inline')
                    ...
                    @endunless
                    <div class="table-row bg-gray-400 divide-x divide-gray-100">
                        @foreach($this->columns as $index => $column)
                            @if($column['hidden'])
                                @if($hideable === 'inline')
