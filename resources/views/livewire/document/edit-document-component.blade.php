<div>
    <!-- Modal -->
    <div wire:ignore.self class="modal modal-message fade" style="width: 100%; margin-left: -10px;" id="documentUModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                    ...
                                    <table class="table table-invoice" id="DetalleServicio" name="DetalleServicio">
                                        ...
                                        <tbody>
                                            ...
                                            <tr class="gradeU">
                                                ...
                                                <td width="7%" class="text-center"><a title="Cambiar Descuento" data-toggle="modal" data-target="#changeDiscountUModal" wire:click="editDiscount({{$index}})" class="text-blue">{{ number_format($line['discount'],2,'.',',')  }}</a></td>
                                                <td width="7%" class="text-center"><a title="Cambiar Impuesto" data-toggle="modal" data-target="#changeTaxUModal" wire:click="editTax({{$index}})" class="text-blue">{{ number_format(((isset($line['tax']['mount']))?$line['tax']['mount']:0),2,'.',',') }}</a></td>
                                                <td width="7%" class="text-center"><a title="Cambiar Exoneraciòn" data-toggle="modal" data-target="#changeTaxUModal" wire:click="editTax({{$index}})" class="text-blue">{{ number_format(((isset($line['tax']['exoneration']['AmountExoneration'])?$line['tax']['exoneration']['AmountExoneration']:0)),2,'.',',') }}</a></td>
                                            ...
                                            <div class="invoice-price-row row">
                                                <div class="sub-price col-md-2">
                                                    ...
                                                    <span class="text-inverse">
                                                        <h5>{{ (isset($subtotal))?number_format($subtotal,2,'.',','):0 }}</h5>
                                                    </span>
                                                </div>
    ...
    <script>
        window.addEventListener('change_discount_modal_hide', event => {
            $('#changeDiscountUModal').modal('hide');
        });
        window.addEventListener('change_tax_modal_hide', event => {
            $('#changeTaxUModal').modal('hide');
        });
        document.addEventListener('DOMContentLoaded', function() {
            ...
            $("#documentUModal").on("hidden.bs.modal", function() {
                Livewire.emit('cleanInputs')
            });
            $('#documentUModal').on('shown.bs.modal', function() {
                $('#name_product').focus();
            });
    ...
    @include('livewire.document.changeDiscountU')
    @include('livewire.document.changeTaxU')
</div>