    ...
    <main>
        <div id="details" class="clearfix">
            <div id="client">
                ...
                <div class="name">{{ $client->name_client }}</div>
            ...
            <div id="invoice">
                ...
                @endif
                @if($seller!='')
                <div class="date">Vendedor: {{ $seller->name_seller }}</div>
                @endif
            </div>
        </div>
        <table border="0" cellspacing="0" cellpadding="0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="price">{{ $detail->code }}</td>
                    <td class="desc">{{ $detail->detail }}</td>
                    <td class="unit">{{ $detail->sku }}</td>
                    <td class="qty">{{ number_format($detail->qty,2,'.',',') }}</td>
                    <td class="price">{{ number_format($detail->price_unid,2,'.',',') }}</td>
                    <td class="discount">{{ number_format($detail->discounts,2,'.',',') }}</td>
                    <td class="tax">{{ number_format((isset(json_decode($detail->taxes)->rate))?json_decode($detail->taxes)->mount:0 ,2,'.',',')}}</td>
                    <td class="exo">{{ number_format((isset(json_decode($detail->taxes)->exoneration->AmountExoneration))?json_decode($detail->taxes)->exoneration->AmountExoneration:0,2,'.',',') }}</td>
                    <td class="total">{{ number_format($detail->total_amount_line,2,'.',',') }}</td>
                </tr>
                @endforeach
            </tbody>
            @if(!$otherCharge)
            <tfoot>
                ...
                <tr>
                    <td colspan="7"></td>
                    <td colspan="2">DESCUENTO TOTAL</td>
        ...
        <table border="0" cellspacing="0" cellpadding="0">
            ...
            <tfoot>
                ...
                <tr>
                    <td colspan="4"></td>
                    <td colspan="1">DECUENTO TOTAL</td>
                    <td>{{ number_format($document->total_discount,2,'.',',') }}</td>
                </tr>
                <tr>
                    <td colspan="4"></td>
