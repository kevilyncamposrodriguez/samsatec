    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <table id="" class="table table-striped table-bordered table-td-valign-middle">
                    ...
                    <tbody>
                        ...
                        <tr class="gradeU">
                            <td width="25%" class="text-center">{{ (isset($fact->key))?substr($fact->key,21,20):0 }}</td>
                            <td width="25%" class="text-center">{{ number_format(((isset($fact->pending_amount))?$fact->pending_amount:0),2,'.',',') }}</td>
                            <td width="25%" class="text-center"><input class="form-control" type="number" value="0" wire:model="pay.{{ $index }}"></td>
                            <td width="25%" class="text-center">{{ number_format($fact->pending_amount - ((isset($pay[$index]) && is_numeric($pay[$index]))?$pay[$index]:0),2,'.',',') }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
                    ...
                    <thead>
                        <tr style="background-color: #192743;" class="text-white">
                            <th class="text-nowrap" colspan="2">
                                <h4>Monto a cancelar: ₡ {{ $totalPay }}</h4>
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr class="gradeU">
                            <td width="25%" class=""><b class="uppercase">Cuenta a debitar:</b></td>
                        ...
                        <tr class="gradeU">
                            <td width="25%" class=""><b class="uppercase">Cuenta de proveedor:</b></td>
                            <td width="25%" class="text-center">

                                <select class="form-control" name="id_countP" id="id_countP" wire:model="allProviderAccounts">
                                    <option style="color: black;" value="">Ninguna</option>
                                    @foreach ($allProviderAccounts as $index => $acountP)
                                    <option style="color: black;" value="{{ $acountP->id }}">{{ $acountP->description }} - {{ $acountP->account }}</option>
                                    @endforeach
                                </select> @error('id_countP') <span class="text-red-500">{{ $message }}</span>@enderror
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div class="modal-footer">
                ...
                <button type="button" wire:click.prevent="storePay()" onclick="this.disabled=true;" class="btn btn-primary close-modal">Realizar Pagos</button>
            </div>
        </div>
    </div>
</div>