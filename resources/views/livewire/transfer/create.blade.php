<!-- Modal -->
<div wire:ignore.self class="modal fade " id="transferModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Crear Transferencia</h5>
                ...
                <form class="form-horizontal form-bordered">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-4" wire:ignore>
                            <select class="form-control " name="id_count_d" id="id_count_d">
                        ...
                        <div class="col-4" wire:ignore>
                            <select class="form-control " name="id_count_c" id="id_count_c">
                                @foreach($allCounts as $count)
                                <option style="color: black;" value="{{ $count->id }}">{{ $count->name }}</option>
                                @endforeach
                            </select>
...
<script>
    window.addEventListener('transfer_modal_hide', event => {
        $('#transferModal').modal('hide');
    });
    document.addEventListener('DOMContentLoaded', function() {
        $('#id_count_c').select2();
        $('#id_count_c').on('change', function(e) {
            livewire.emit('changeCountC', $('#id_count_c').select2("val"))
        });
        $('#id_count_d').select2();
        $('#id_count_d').on('change', function(e) {
            livewire.emit('changeCountD', $('#id_count_d').select2("val"))
        });
    });
    //manejo de clientes
    window.addEventListener('countC-updated', event => {
        $("#id_count_c").select2().val(event.detail.newValue).trigger('change.select2');
    })
    window.addEventListener('countD-updated', event => {
        $("#id_count_d").select2().val(event.detail.newValue).trigger('change.select2');
    })
</script>