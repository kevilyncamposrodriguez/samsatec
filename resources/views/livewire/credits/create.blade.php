<!-- Modal -->
<div wire:ignore.self class="modal fade " id="creditModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
            ...
            <div class="modal-body">
                <form class="form-horizontal form-bordered">
                    ...
                    <div class="form-group row m-b-10">
                        ...
                        <label class="col-lg-2 col-form-label">Gastos Formalización: <span class="text-danger"></span></label>
                        <div class="col-xs-4 mb-4 mb-sm-0">
...
<script>
    window.addEventListener('credit_modal_hide', event => {
        $('#creditModal').modal('hide');
    });
    document.addEventListener('DOMContentLoaded', function() {
        $('#currency').select2();
        $('#currency').on('change', function(e) {
            livewire.emit('changeCurrency', $('#currency').select2("val"))
        });
    });
    //manejo de clientes
    window.addEventListener('currency-updated', event => {
        $("#currency").select2().val(event.detail.newValue).trigger('change.select2');
    })
</script>